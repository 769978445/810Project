...
import os
from SSW810_P8 import file_reading_gen
from collections import defaultdict
import unittest
import sqlite3
import warnings

DB_FILE = 'D:/PycharmProjects/Week11'
...
class Repository:
    ...
    def __init__(self, path_dir):
        ...
        try:
            ...
            try:
                self.read_student(os.path.join(path_dir, 'students.txt'))
                self.match_major()
            except ValueError as err:
                print(err)
            else:
                try:
                    self.read_instructor(os.path.join(path_dir, 'instructors.txt'))
                except ValueError as err:
                    print(err)
                else:
                    try:
                        self.read_grade(os.path.join(path_dir, 'grades.txt'))
                    except ValueError as err:
                        print(err)

    def read_student(self, path):
        """Read student from file, get their cwid, name and major, and create a new instance of Student class"""
        try:
    ...
    def read_major(self, path):
        ...
        for major, flag, course in file_reading_gen(path, 3, sep='\t', header=False):
            if major in self._major:  # If this major already have an instance
                ...
                self._major[major] = Major(
                    major)  # If this major does not have an intance yet, create a new instance first
                if flag == 'R':
                    self._major[major].add_required(course)  # Add required courses into instance
                elif flag == 'E':
                    self._major[major].add_elective(course)  # Add elective courses into instance
    ...
    def match_major(self):
        ...
        for cwid in self._student.keys():  # Iterate through all instance of Student
            if self._student[
                cwid]._major in self._major.keys():  # Catch bad vakues if one student's major is not in Major class
                for major in self._major.keys():  # Iterate through all instance of Major
                    if self._student[cwid]._major == major:  # Match each student with his/her major
                        self._student[cwid].add_majorlist(self._major[major]._required, self._major[
                            major]._elective)  # Add required course and elective course into instance of student

            else:
                raise ValueError(
                    "Major was not matched with Student's record")  # Raise exception if one student's major is not in Major class
        ...
        try:
            for scwid, course, grade, icwid in file_reading_gen(path, 4, sep='\t', header=False):
                ...
                if icwid in self._instructor:
                    self._instructor[icwid].add_course(
                        course)  # Add course and how many students into instence of instructor
                else:
...
class Student:
    ...
    pt_labels = ['CWID', 'NAME', 'MAJOR', 'COMPLETED COURSES', 'REMAINING REQUIRED',
                 'REMAINING ELECTIVE']  # Filed names
    ...
    def add_course(self, course, grade):
        ...
        self._course_completed = [i for i in self._course_took if
                                  self._course_took[i] in ['A', 'A-', 'B+', 'B', 'B-', 'C+',
                                                           'C']]  # Only courses with a grade better than 'C' will be determined as completed
        self._remaning_required = [i for i in self._major_required if
                                   i not in self._course_completed]  # Courses are required but not yet completed
        self._remaning_elective = 'None' if len(set(self._course_completed) & set(
            self._major_elective)) > 0 else self._major_elective  # Shows 'None' if at least one elective course has been taken, otherwise, shows list of elective courses
...
class Instructor:
    ...
    def add_course(self, course):
        """Add course amd how many students in that course into instance of Instructor"""
        self._course[course] += 1

    def get_course(self):
        """Yield courses and how many students for instance of Instructor"""
        for key in self._course.keys():
            yield key, self._course[key]  # Generator since one instructor might have move than one course

    # def pt_row(self):
    # """Provide information that to be added into PrettyTable for instance of Instructor"""
    # for key in self._course.keys():
    # yield self._cwid, self._name, self._department, key, self._course[key]
    def pt_row(self):
        """Get information from DB and return for adding into PrettyTable"""
        query = (f"""select instructors.CWID, instructors.Name, instructors.Dept, grades.Course, count(*) as 'Students' 
                from grades join instructors on grades.Instructor_CWID = instructors.CWID where instructors.CWID = '{self._cwid}'   
                group by Course order by count(*) DESC""")  # Gather information form DB by self._cwid and yield
...
def main():
    ...
    Stevens = Repository(path_dir)  # Create a instance of University for Stevens
...
class AuotoTest(unittest.TestCase):
    ...
    def test_Stevens(self):
        ...
        path_dir = 'D:/PycharmProjects/Week11'
