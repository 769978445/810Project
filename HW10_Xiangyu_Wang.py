...
class Repository:
    """Store all the information that came from same University"""
        ...
        try:
            for cwid, name, major in file_reading_gen(path, 3, sep=';', header=False):
                if cwid in self._student:
                    print(f'Student{cwid} already in file!')  # Catch duplicate instance of student
                else:
                    self._student[cwid] = Student(cwid, name, major)
        except ValueError as error:
            raise ValueError(error)  # Raise a exception when encountering a bad data

    def read_instructor(self, path):
        """Read instructor form file, get their cwid, name and department, and create a new instance"""
        try:
            for cwid, name, department in file_reading_gen(path, 3, sep=';', header=False):
                if cwid in self._instructor:
                    print(f'Instructor{cwid} already in file!')  # Catch duplicated instance of instructor
                else:
    ...
    def read_major(self, path):
        """Read major file, create new instance for every major, add required courses and elective courses into
        instance """
        for major, flag, course in file_reading_gen(path, 3, sep=';', header=False):
            if major in self._major:  # If this major already have an instance
                ...
                # If this major does not have an instance yet, create a new instance first
    ...
    def match_major(self):
        ...
        for cwid in self._student.keys():  # Iterate through all instance of Student
            if self._student[
                # Catch bad vakues if one student's major is not in Major class
                cwid]._major in self._major.keys():
                ...
                raise ValueError(
                    "Major was not matched with Student's record")  # Raise exception if one student's major is not
                # in Major class
...
def main():
    ...
    Stevens = Repository(path_dir)  # Create a instence of University for Stevens
    try:
...
class AuotoTest(unittest.TestCase):
    ...
    def test_Stevens(self):
        ...
        path_dir = 'D:/PycharmProjects/Week10'
        ...
        except1 = "Student: 11714, Name: Morton, A, Major: SYEN, Completed Course: ['SYS 611', 'SYS 645'], Remaining " \
                  "Required Courses: ['SYS 612', 'SYS 671', 'SYS 800'], Remaining Elective Courses: ['SSW 540', " \
                  "'SSW 565', 'SSW 810'] "
        except2 = "Instructor: 98765, Name: Einstein, A, Department: SFEN, defaultdict(<class 'int'>, {'SSW 567': 4, " \
                  "'SSW 540': 3}) "
